---
name: sugar-custom-field-type
description: Create a SugarCRM custom field TYPE (e.g., highlight, color picker, slider) with a PHP class, JS view, edit/detail Handlebars templates, and a vardef referencing the type via `custom_type`. Use when a field needs custom rendering or interaction beyond standard varchar/text.
when_to_use:
  - "create a highlight field type"
  - "build a color picker field"
  - "custom field with handlebars template"
  - "field type with javascript view"
  - "extend SugarFieldBase"
not_for:
  - Simple varchar/text/int/bool fields — use sugar-custom-field
  - Adding an existing field to a view — use sugar-ui-customization
related_skills:
  - sugar-custom-field
...
Use this skill when a developer needs a custom field TYPE — meaning a brand-new rendering/interaction class (highlight, color picker, slider, gauge) that requires a PHP `SugarFieldBase` subclass, a JS Backbone view, and Handlebars edit/detail templates. The `custom_type` vardef property references the new field type. If the developer only needs a varchar/text/int field, use `[[sugar-custom-field]]` instead.
...
| Symptom | Fix |
|---------|-----|
| Field renders as plain text | Confirm `.js` registered the new `fieldType` and templates exist under `custom/Extension/fields/<fieldtype>/templates/` |
| Edit input missing | `setEditValue` not implemented or template missing required input element |
| Save not picking up value | `getDisplayValue` not returning the value the input holds |

## References

- `[[sugar-custom-field]]` — simpler Vardef-only fields
- `[[sugar-ui-customization]]` — broader view customization
