---
name: sugar-scheduler
description: Add a SugarCRM scheduled job via the Extension Framework — registers $job_strings and a job class with a static method. Use when a developer wants cron-style recurring work (cleanup, sync, batch email).
when_to_use:
  - "add a scheduled job to Sugar"
  - "cron job that runs every hour/day"
  - "batch process records nightly"
  - "register a job under Schedulers Ext/ScheduledTasks"
not_for:
  - One-time bean events — use sugar-logic-hook
  - External webhooks INTO Sugar — use sugar-rest-endpoint
related_skills:
  - sugar-logic-hook
...
Use this skill when a developer needs Sugar to run code on a schedule (cron). The job is registered via the Extension Framework under `custom/Extension/modules/Schedulers/Ext/ScheduledTasks/<filename>.php` and implemented in `custom/modules/Schedulers/<ClassName>.php`. Sugar invokes a static method on the class. Always log start/end, catch exceptions, and return boolean — never throw.
...
| Symptom | Fix |
|---------|-----|
| Job doesn't appear in Admin → Scheduler | Run Quick Repair & Rebuild; confirm Ext/ScheduledTasks/ path |
| Job fails silently | Wrap in try/catch — never throw; log to `$GLOBALS['log']` |
| Cron format off-by-one | Sugar accepts standard cron (5 fields, `*/15`, `0 */6`) |

## References

- `[[sugar-external-resource-client]]` — outbound HTTP from jobs
- `[[sugar-logic-hook]]` — event-driven alternative
