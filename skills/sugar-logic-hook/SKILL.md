---
name: sugar-logic-hook
description: Generate a SugarCRM logic hook MLP using the Extension Framework — namespaced class, hook definition file, and ext/LogicHooks registration. Use when a developer wants to react to before_save, after_save, before_delete, before_relationship_add, etc.
when_to_use:
  - "add a logic hook to Accounts"
  - "react to before_save / after_save / before_delete on a module"
  - "trigger a webhook when a record is saved"
  - "fire custom PHP on relationship add/remove"
  - "register a hook via Extension Framework"
not_for:
  - Scheduled jobs (cron) — for that use sugar-scheduler
  - Custom REST endpoints — for that use sugar-rest-endpoint
  - Outbound HTTP details — for that use sugar-external-resource-client
related_skills:
...
Use this skill any time a SugarCRM developer needs to run custom PHP in response to a bean event (`before_save`, `after_save`, `before_delete`, `after_delete`, `before_restore`, `after_restore`, `before_relationship_add`, `after_relationship_add`, `before_relationship_delete`, `after_relationship_delete`, `process_record`, `after_retrieve`). Hooks must be distributed via the Extension Framework, never by editing `logic_hooks.php` directly. For HTTP calls inside the hook, cross-reference `[[sugar-external-resource-client]]`.
...
| Symptom | Fix |
|---------|-----|
| Hook never fires | Confirm file is under `custom/Extension/modules/<Module>/Ext/LogicHooks/` and not in `logic_hooks.php` directly |
| Class not found at runtime | Namespace path must match file path (`Sugarcrm\Sugarcrm\custom\modules\<Module>\<ClassName>` → `custom/modules/<Module>/<ClassName>.php`) |
| Hook fires twice | Idempotency — guard with `fetched_row` comparison |
| Hook halts execution silently | Check Sugar `sugarcrm.log` for thrown exception |

## References

- `[[sugar-external-resource-client]]` — outbound HTTP from hooks
- `[[sugar-package-build]]` — wrap into a deployable zip
- https://support.sugarcrm.com/documentation/sugar_developer/sugar_developer_guide_25.1/architecture/logic_hooks/
