---
name: sugar-ui-customization
description: Customize SugarCRM Sidecar UI — views, layouts, dashlets, subpanels, metadata — using the Extension Framework. JS Backbone view + Handlebars template + optional CSS + optional metadata override.
when_to_use:
  - "customize a record view"
  - "add a dashlet"
  - "override a subpanel layout"
  - "create a Sidecar view with backbone + handlebars"
  - "tweak module metadata for buttons/panels"
not_for:
  - Adding a custom field type — use sugar-custom-field-type
  - Adding a simple field — use sugar-custom-field
  - Editing viewdefs precisely (Sidecar safe editing) — use sugar-viewdef-editing
related_skills:
...
Use this skill for Sidecar UI customization — custom views, layouts, dashlets, subpanels — registered via the Extension Framework. The deliverable is typically a Backbone view JS + Handlebars HBS template + optional CSS + optional metadata override. For safe in-place editing of an existing Sidecar viewdef (where renumbering numeric keys matters), prefer `[[sugar-viewdef-editing]]`.
...
| Symptom | Fix |
|---------|-----|
| View doesn't render | Run Quick Repair; confirm `define()` path matches file path |
| Template not found | Confirm `.hbs` location matches `clients/base/views/<view>/` |
| Editing existing viewdef breaks | Use `[[sugar-viewdef-editing]]` — numeric keys must be renumbered |

## References

- `[[sugar-viewdef-editing]]` — safe edits to existing Sidecar viewdefs
- `[[sugar-custom-field-type]]` — for field-type-specific rendering
