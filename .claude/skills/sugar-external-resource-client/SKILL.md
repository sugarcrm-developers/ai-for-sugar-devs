---
name: sugar-external-resource-client
description: Outbound HTTP from SugarCRM PHP — MANDATORY use of `Sugarcrm\Sugarcrm\Security\HttpClient\ExternalResourceClient` constructor (NOT getInstance). Never curl, file_get_contents, fopen, or stream_get_contents.
when_to_use:
  - "send a webhook from PHP"
  - "POST JSON to an external API"
  - "call out to an integration endpoint"
  - "make HTTP requests inside a logic hook or scheduler"
  - "ExternalResourceClient usage"
not_for:
  - Receiving HTTP into Sugar — use sugar-rest-endpoint
related_skills:
  - sugar-logic-hook
  - sugar-scheduler
  - sugar-rest-endpoint
...
Any time PHP inside a SugarCRM customization (logic hook, scheduler, REST handler) needs to make an outbound HTTP request. ALL such requests MUST go through `Sugarcrm\Sugarcrm\Security\HttpClient\ExternalResourceClient` via `new ExternalResourceClient()`. Never use `curl`, `file_get_contents`, `fopen`, or `stream_get_contents`. There is no `getInstance()` static factory — that's a frequent mistake; the constructor is the only entry point.
...
| Symptom | Fix |
|---------|-----|
| `Call to undefined method ::getInstance()` | Use the constructor: `new ExternalResourceClient()` |
| SSL/TLS rejection on internal URL | Sugar enforces allow-lists for outbound URLs; whitelist via Admin |
| Timeout default too high | Pass `'timeout' => 5` explicitly |

## References

- `[[sugar-logic-hook]]` — most common caller
- `[[sugar-scheduler]]` — long-running jobs that hit APIs
- `[[sugar-rest-endpoint]]` — receiving end (not sending)
- https://support.sugarcrm.com/documentation/sugar_developer/sugar_developer_guide_14.0/integration/externalresourceclient/
