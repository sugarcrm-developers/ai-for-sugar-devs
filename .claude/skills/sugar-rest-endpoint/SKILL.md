---
name: sugar-rest-endpoint
description: Add a custom REST API endpoint to SugarCRM via the Extension Framework. Builds a SugarApi/ModuleApi subclass with registerApiRest() and handler methods. Use when a developer wants `/custom/<resource>` GET/POST/PUT/DELETE/PATCH routes.
when_to_use:
  - "create a custom REST endpoint"
  - "add API route /custom/<something>"
  - "expose a module action via REST"
  - "extend SugarApi"
  - "build a webhook receiver"
not_for:
  - Calling OUT to external APIs — use sugar-external-resource-client
  - Reacting to bean events — use sugar-logic-hook
related_skills:
...
Use this skill when a developer needs to expose custom HTTP endpoints from Sugar — typically subclassing `SugarApi` or `ModuleApi`, registering paths via `registerApiRest()`, and implementing handler methods that return JSON. Use the Extension Framework so the endpoint installs cleanly via Module Loader and is upgrade-safe.
...
| Symptom | Fix |
|---------|-----|
| 404 on new endpoint | Run Quick Repair & Rebuild; registration cached |
| 401 unexpected | Auth check rejecting — confirm `$GLOBALS['current_user']` and acl entries |
| Invalid parameter accepted | Add explicit `SugarApiExceptionInvalidParameter` checks |

## References

- `[[sugar-external-resource-client]]` — when your endpoint forwards to an outside API
- `[[sugar-logic-hook]]` — event-driven alternative when no caller will hit a URL
